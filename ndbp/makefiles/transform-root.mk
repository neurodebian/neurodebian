...
define add_patch
	...
	set -e; \
	. '$(ndtool_rootdir)/ndbp/helpers/patch-file-helper'; \
...
common-neurodebian:
	...
	# Replace maintainer and add dependency on neurodebian-popularity-contest
	sed -i \
		-e 's/^Maintainer:.*$$/Maintainer: NeuroDebian Team <team@neuro.debian.net>\nX-Original-&/' \
		-e 's/\(^Depends:\) */\1 neurodebian-popularity-contest, /g' \
		debian/control
	# Point to correct synthetic neurodebian/* branch in 'debian/control'
	sed -i \
		-e '/^Vcs-Git: /{s/ -b .*$$//; s|^.*$$|& -b $(NEURODEBIAN_BRANCH)|}' \
		debian/control
	# Point to correct synthetic neurodebian/* branch in existing 'debian/gbp.conf'
	# or create minimal one if not
	if [ -f debian/gbp.conf ]; then \
		sed -i \
			-e 's|debian-branch = .*$$|debian-branch = $(NEURODEBIAN_BRANCH)|' \
			-e 's|debian-tag = .*$$|debian-tag = neurodebian/%(version)s|' \
			debian/gbp.conf; \
	else \
		echo '# Configuration file for git-buildpackage and friends' >debian/gbp.conf; \
		echo '' >>debian/gbp.conf; \
		echo '[DEFAULT]' >>debian/gbp.conf; \
		echo 'filter = */.git*' >>debian/gbp.conf; \
		echo 'debian-branch = $(NEURODEBIAN_BRANCH)' >>debian/gbp.conf; \
		echo 'debian-tag = neurodebian/%(version)s' >>debian/gbp.conf; \
	fi
