...
.internal-export-%:
	...
	. '$(ndtool_rootdir)/ndbp/helpers/distinfo-file-helper'; \
	...
	if command -v gbp 1>/dev/null; then \
		'$(SHELL)' '$(ndtool_rootdir)/ndbp/build-steps/export-source-step' \
			'$(CURDIR)' \
			"$${PKGBRANCH}" \
		...
		'$(PODMAN)' run \
			...
			"$${TEMP_OPTS}" \
			-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
			-v '$(ndtool_rootdir)/ndbp/build-steps/export-source-step:/export-source-step:ro' \
			-v '$(CURDIR)/:/src:ro' \
...
.internal-builddep-%:
	...
	# 4. If ARCH is NATIVE_ARCH and 'source' is among distinfo architectures,
	#    continue, else check if ARCH is defined among package's binary
	#    artifacts (and SKIP step if not)
	...
	. '$(ndtool_rootdir)/ndbp/helpers/debian-control-file-helper'; \
	. '$(ndtool_rootdir)/ndbp/helpers/filter-helper'; \
	SKIP_STEP=0; \
	if [ "$${ARCH}" = "$(NATIVE_ARCH)" ]; then \
		. '$(ndtool_rootdir)/ndbp/helpers/distinfo-file-helper'; \
		DIST_ARCHES="$$(distinfo_get_architectures "$${VENDOR}-$${DIST}" 'true')"; \
		if [ -z "$$(filter_common 'source' "$${DIST_ARCHES}")" ]; then \
			SKIP_STEP=1; \
		fi; \
	else \
	...
	'$(PODMAN)' run \
		...
		"$${TEMP_OPTS}" \
		-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/get-build-dep-step:/get-build-dep-step:ro' \
...
.internal-reprotestdep-%:
	...
	'$(PODMAN)' run \
		...
		"$${TEMP_OPTS}" \
		-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/get-reprotest-dep-step:/get-reprotest-dep-step:ro' \
...
build-%:
	...
	'$(PODMAN)' run \
		...
		"$${TEMP_OPTS}" \
		-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/build-package-step:/build-package-step:ro' \
		-v "$(OUTDIR)/temp/export/$${VENDOR}/$${DIST}:/src:ro" \
...
lintian-%:
	...
	'$(PODMAN)' run \
		...
		"$${TEMP_OPTS}" \
		-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/lintian-package-step:/lintian-package-step:ro' \
...
reprotest-%:
	...
	'$(PODMAN)' run \
		...
		"$${TEMP_OPTS}" \
		-v '$(ndtool_rootdir)/ndbp/build-steps/restart-shell:/restart-shell:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/setarch-helper:/usr/bin/setarch-helper:ro' \
		-v '$(ndtool_rootdir)/ndbp/build-steps/reprotest-package-step:/reprotest-package-step:ro' \
		-v "$(OUTDIR)/temp/export/$${VENDOR}/$${DIST}:/src:ro" \
